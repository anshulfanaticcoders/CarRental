...
class XDriveService
{
    ...
    {
        $config = config('services.xdrive');

        if (empty($config['base_url']) || empty($config['key_hack'])) {
            throw new \Exception('XDrive credentials are not configured correctly.');
        }
    ...
    public function searchRez(
        ...
        if ($this->username === '' || $this->password === '') {
            Log::warning('XDrive search attempted without username/password.');
            return [];
        }
        ...
        if (is_array($first) && isset($first['success']) && strtolower((string) $first['success']) === 'false') {
            Log::warning('XDrive search returned error', [
    ...
    {
        if ($this->username === '' || $this->password === '') {
            Log::warning('XDrive reservation attempted without username/password.');
            return null;
        }
    ...
    {
        if ($this->username === '' || $this->password === '') {
            Log::warning('XDrive cancel attempted without username/password.');
            return null;
        }
    ...
    private function get(string $endpoint, array $params): ?array
    {
        if ($this->isCircuitOpen()) {
            Log::warning('XDrive circuit breaker open', ['endpoint' => $endpoint]);
            return null;
        }
    ...
    private function executeWithRetry(callable $callback, string $operation): ?array
    {
        ...
        for ($attempt = 1; $attempt <= $this->maxRetries; $attempt++) {
            try {
                ...
                Log::warning('XDrive API attempt failed', [
                    'operation' => $operation,
                    'attempt' => $attempt,
                    'error' => $e->getMessage(),
                ]);
        ...
        Log::error('XDrive API operation failed after retries', [
            'operation' => $operation,
    ...
    private function isCircuitOpen(): bool
    {
        if ($this->circuitOpen) {
            if ($this->lastFailureTime !== null && (time() - $this->lastFailureTime) > $this->circuitBreakerTimeout) {
                $this->circuitOpen = false;
                $this->failures = 0;
                Log::info('XDrive circuit breaker reset after timeout');
                return false;
            }
            return true;
        }
