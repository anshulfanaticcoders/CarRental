  ...
  <soap:Body>
    ...
    public function parseVehicleResponse($xmlResponse): array
    {
        ...
        try {
            ...
            // Look for vehicle information in the response
            $vehicleNodes = $xmlObject->xpath('//ota:Vehicle');
            ...
            if (!empty($vehicleNodes)) {
                foreach ($vehicleNodes as $vehicleNode) {
                    $vehicle = [
                        ...
                        'sipp_code' => (string) $vehicleNode['Code'] ?? '',
                        'transmission' => (string) $vehicleNode['TransmissionType'] ?? '',
                        'fuel' => (string) $vehicleNode['FuelType'] ?? '',
                        'seating_capacity' => (int) ($vehicleNode['PassengerQuantity'] ?? 0),
                        'doors' => (int) ($vehicleNode['DoorCount'] ?? 0),
                        'image' => (string) $vehicleNode['PictureURL'] ?? '',
                        'price_per_day' => 0,
                        'currency' => 'EUR',
                        'availability' => true,
                    ];
                    ...
                    $chargeNodes = $vehicleNode->xpath('.//ota:VehicleCharge');
                    if (!empty($chargeNodes)) {
                        foreach ($chargeNodes as $chargeNode) {
                            if (isset($chargeNode['Amount'])) {
                                $vehicle['total_amount'] = (float) $chargeNode['Amount'];
                                $vehicle['currency'] = (string) $chargeNode['CurrencyCode'] ?? 'EUR';
                                break;
                            }
                        }
                    }
            ...
            Log::error('Error parsing Locauto vehicle response: ' . $e->getMessage());
            return [];
        }
    }
