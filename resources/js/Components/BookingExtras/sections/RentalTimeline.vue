...
<template>
    <section class="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 fade-in-up" style="animation-delay:0.05s">
        <div class="grid grid-cols-1 gap-4" :class="isOneWay ? 'sm:grid-cols-2' : ''">
            <!-- Pickup -->
            ...
            <div v-if="!isOneWay" class="rounded-xl border border-emerald-100 bg-emerald-50/45 px-4 py-3 sm:ml-[3.25rem]">
                <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <div class="min-w-0">
                        <span class="text-xs font-bold text-emerald-700 uppercase tracking-wider">Return</span>
                        <p class="text-sm font-bold text-gray-900 leading-snug mt-0.5">Same as pickup</p>
                    </div>
                    <p class="text-xs text-gray-600 flex items-center gap-1.5">
                        <svg class="h-3.5 w-3.5 text-emerald-700" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                        {{ dropoffDate }} <span class="text-emerald-200">|</span> {{ dropoffTime }}
                    </p>
                </div>
            </div>
