    ...
    <MyProfileLayout>
        <div class="container mx-auto px-3 sm:px-4 lg:px-6 py-4 sm:py-6 space-y-4 sm:space-y-6">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    ...
                    <Link :href="route('vehicles.create', { locale: usePage().props.locale })" class="w-full sm:w-auto">
                        <Button size="sm" class="flex items-center justify-center gap-2 w-full sm:w-auto">
                            ...
                            <span class="hidden sm:inline">{{ _t('vendorprofilepages', 'add_new_vehicle_button') }}</span>
            ...
            <div class="space-y-4 sm:space-y-6 md:grid md:grid-cols-2 lg:grid-cols-4 md:gap-6">
                ...
                <div class="relative rounded-xl shadow-lg transition-all duration-200 hover:shadow-xl hover:scale-[1.02] border unified-category-card total-vehicles-card">
                ...
                <div v-for="(category, index) in categoryStatsData" :key="category.name"
                    ...
                    <div class="category-card-content">
                        ...
                        <p :class="[getCategoryLabelClass(index), 'category-card-description']">{{ category.count === 1 ? 'Vehicle' : 'Vehicles' }}</p>
                    </div>
                </div>
            </div>
            ...
            <div v-if="selectedVehicleIds.length > 0" class="bulk-actions-bar animate-in">
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bulk-actions-content">
                    ...
                    <div class="flex flex-col sm:flex-row gap-2 sm:gap-3 w-full sm:w-auto bulk-actions-buttons">
                        ...
                        >
                            ...
                            <span class="hidden sm:inline">{{ _t('vendorprofilepages', 'delete_selected_button') }}</span>
            ...
            <div class="relative w-full max-w-md sm:max-w-lg lg:max-w-xl">
                <Search class="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 sm:h-5 sm:w-5 text-muted-foreground" />
                <Input
                    v-model="searchQuery"
                                ...
                                <TableCell class="whitespace-nowrap px-4 py-3 action-cell">
                                    <div class="flex justify-end gap-2 min-w-[120px] action-buttons-fixed">
                                        <Link :href="route('current-vendor-vehicles.edit', { locale: usePage().props.locale, 'current_vendor_vehicle': vehicle.id })">
                                            <Button size="sm" variant="outline" class="flex items-center gap-1 whitespace-nowrap">
                                                <Edit class="w-3 h-3" />
                    ...
                    >
                        ...
                        <div class="flex items-start gap-3 mb-4">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex flex-wrap items-center gap-1.5 mb-2">
                                    <Badge variant="outline" class="text-xs whitespace-nowrap flex-shrink-0">#{{ vehicle.id }}</Badge>
                                    <span class="text-xs text-muted-foreground whitespace-nowrap flex-shrink-0">
                                        {{ (pagination.current_page - 1) * pagination.per_page + index + 1 }}
                                    </span>
                                    <Badge :variant="getStatusBadgeVariant(vehicle.status)" class="capitalize text-xs whitespace-nowrap flex-shrink-0">
                                        {{ vehicle.status }}
                                    </Badge>
                                </div>
                            ...
                            <Link :href="route('vehicle.show', { locale: usePage().props.locale, id: vehicle.id })" class="flex-shrink-0">
                                <div class="relative group car-image-container">
                                    <img
                                        :src="getPrimaryImage(vehicle)"
                                        :alt="_t('vendorprofilepages', 'alt_no_image')"
                        ...
                        <div class="grid grid-cols-2 gap-3 mb-4">
                            <div class="space-y-1">
                                ...
                                <div class="flex items-center">
                                    <Badge variant="secondary" class="text-xs truncate max-w-full">{{ vehicle.transmission }}</Badge>
                                </div>
                            </div>
                            <div class="space-y-1">
                                <span class="text-xs font-medium text-muted-foreground block">Fuel</span>
                                <div class="flex items-center">
                                    <Badge variant="secondary" class="text-xs truncate max-w-full">{{ vehicle.fuel }}</Badge>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="flex items-center gap-1">
                                <MapPin class="w-3 h-3 text-muted-foreground flex-shrink-0" />
                                <span class="text-sm text-gray-700 dark:text-gray-300 line-clamp-2">{{ vehicle.full_vehicle_address }}</span>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
                            <div class="space-y-1">
                                ...
                                <div v-if="vehicle.benefits && (vehicle.benefits.limited_km_per_day_range || vehicle.benefits.limited_km_per_week_range || vehicle.benefits.limited_km_per_month_range)" class="text-xs space-y-1">
                                    <span v-if="vehicle.benefits.limited_km_per_day_range > 0" class="block text-green-600">
                                        {{ vehicle.benefits.limited_km_per_day_range }}/day
                                    </span>
                                    <span v-if="vehicle.benefits.limited_km_per_week_range > 0" class="block text-green-600">
                                        {{ vehicle.benefits.limited_km_per_week_range }}/week
                                    </span>
                                    <span v-if="vehicle.benefits.limited_km_per_month_range > 0" class="block text-green-600">
                                        {{ vehicle.benefits.limited_km_per_month_range }}/month
                                    </span>
                                </div>
                                <span v-else class="text-xs text-green-600">{{ _t('vendorprofilepages', 'unlimited_km_text') }}</span>
                            </div>
                            <div class="space-y-1">
                                <span class="text-xs font-medium text-muted-foreground block">Cancellation</span>
                                <div v-if="vehicle.benefits && (vehicle.benefits.cancellation_available_per_day || vehicle.benefits.cancellation_available_per_week || vehicle.benefits.cancellation_available_per_month)" class="text-xs space-y-1">
                                    <span v-if="vehicle.benefits.cancellation_available_per_day" class="block text-blue-600">{{ _t('vendorprofilepages', 'cancellation_day') }}</span>
                                    <span v-if="vehicle.benefits.cancellation_available_per_week" class="block text-blue-600">{{ _t('vendorprofilepages', 'cancellation_week') }}</span>
                                    <span v-if="vehicle.benefits.cancellation_available_per_month" class="block text-blue-600">{{ _t('vendorprofilepages', 'cancellation_month') }}</span>
                                </div>
                                <span v-else class="text-xs text-muted-foreground">{{ _t('vendorprofilepages', 'not_available_text') }}</span>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-3 border-t">
                            ...
                            <div class="flex flex-row gap-2">
                                <Link :href="route('current-vendor-vehicles.edit', { locale: usePage().props.locale, 'current_vendor_vehicle': vehicle.id })" class="flex-1">
                                    <Button size="sm" variant="outline" class="w-full flex items-center justify-center gap-1 text-xs">
                                        <Edit class="w-3 h-3" />
                                        {{ _t('vendorprofilepages', 'edit_button') }}
                                    </Button>
                                </Link>
                        ...
                        <div class="mt-3 pt-3 border-t">
                            <span class="text-xs text-muted-foreground">Created: {{ formatDate(vehicle.created_at) }}</span>
                        </div>
                    </div>
                </div>
            ...
            <div v-else class="rounded-xl border bg-card p-6 sm:p-12 text-center">
                <div class="flex flex-col items-center space-y-4">
                    ...
                    <div class="space-y-2 max-w-md">
                        <h3 class="text-lg sm:text-xl font-semibold text-foreground">{{ _t('vendorprofilepages', 'no_vehicles_found_text') }}</h3>
                        <p class="text-sm sm:text-base text-muted-foreground">Get started by adding your first vehicle to the fleet.</p>
                    </div>
                    <Link :href="route('vehicles.create', { locale: usePage().props.locale })" class="w-full sm:w-auto max-w-xs">
            ...
            <AlertDialog v-model:open="showDeleteModal">
                <AlertDialogContent class="w-[95vw] sm:max-w-md mx-auto max-h-[95vh] overflow-auto">
                    ...
                    <AlertDialogFooter class="flex flex-col sm:flex-row gap-2 sm:gap-3 pt-4">
                        <AlertDialogCancel @click="showDeleteModal = false; vehicleToDelete = null;" class="flex items-center justify-center gap-2 w-full sm:w-auto">
                            <X class="w-4 h-4" />
                            {{ _t('vendorprofilepages', 'cancel_button') }}
                        </AlertDialogCancel>
                        <AlertDialogAction @click="vehicleToDelete ? deleteVehicle() : deleteSelectedVehicles()" class="flex items-center justify-center gap-2 w-full sm:w-auto">
                            <Trash2 class="w-4 h-4" />
                            {{ _t('vendorprofilepages', 'delete_button_general') }}
                        </AlertDialogAction>
                    </AlertDialogFooter>
