                ...
                <section v-show="currentStep === 6" class="vln-step" key="step-6">
                    ...
                    <div v-if="!props.vehicle || !props.vehicle.images || props.vehicle.images.length < 20" class="vln-field-group">
                        <div class="vln-fg-header">
                            ...
                            <div><div class="vln-fg-title">Upload New Images</div><div class="vln-fg-sub">{{ VEHICLE_IMAGE_UPLOAD_HINT }}</div></div>
                        </div>
                        <div class="vln-upload-zone" @dragenter="onDragEnter" @dragleave="onDragLeave" @dragover="onDragOver" @drop="onDrop">
                            ...
                            <div class="vln-upload-hint">{{ VEHICLE_IMAGE_UPLOAD_HINT }}</div>
...
const handleFileUpload = async (e) => {
    ...
    const validationError = await validateVehicleImageFiles(newFiles);
    if (validationError) {
...
const onDrop = async (e) => {
    ...
    if (f.length) {
        ...
        const validationError = await validateVehicleImageFiles(f);
        if (validationError) {
            errorMessage.value = validationError;
            showErrorDialog.value = true;
            return;
...
<style scoped>
/* STEPPER */
.vln-stepper { position: sticky; top: calc(var(--profile-gutter, 1.5rem) * -1); z-index: 25; margin-bottom: 1rem; background: rgba(255,255,255,0.96); backdrop-filter: blur(14px) saturate(1.15); border: 1px solid #e2e8f0; border-radius: 14px; box-shadow: 0 8px 22px rgba(21, 59, 79, 0.08); overflow-x: auto; scrollbar-width: none; }
.vln-stepper::-webkit-scrollbar { display: none; }
.vln-stepper-inner { padding: 0 1rem; display: flex; width: 100%; min-width: 760px; }
