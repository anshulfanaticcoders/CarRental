          ...
          <div class="p-6 sm:px-20 bg-white border-b border-gray-200">
            ...
            <form @submit.prevent="submitForm">
              <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                  <label for="seo_title" class="block text-sm font-medium text-gray-700">Default SEO Title (Required)</label>
                  <input type="text" id="seo_title" v-model="form.seo_title" maxlength="60" class="mt-1 p-2 border-2 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" placeholder="Default SEO title for the page" />
                ...
                <div>
                  <label for="meta_description" class="block text-sm font-medium text-gray-700">Default Meta Description</label>
                  <textarea id="meta_description" v-model="form.meta_description" maxlength="160" rows="3" class="mt-1 p-2 border-2 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" placeholder="Default short and compelling description"></textarea>
                ...
                <div>
                  <label for="keywords" class="block text-sm font-medium text-gray-700">Default Keywords (comma-separated)</label>
                  <input type="text" id="keywords" v-model="form.keywords" class="mt-1 p-2 border-2 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" placeholder="default_keyword1, default_keyword2" />
                ...
                <div class="mt-6 mb-4 border-b border-gray-200">
                  ...
                  <nav class="-mb-px flex space-x-8" aria-label="Tabs">
                    <button
                      ...
                      :key="locale"
                      type="button" @click="currentLocaleTab = locale"
                      :class="[
                ...
                <template v-for="locale in locales" :key="locale">
                  <div v-if="currentLocaleTab === locale">
                    ...
                    <div class="mb-4">
                      <label :for="'trans_seo_title_' + locale" class="block text-sm font-medium text-gray-700">SEO Title [{{ locale.toUpperCase() }}]</label>
                      <input type="text" :id="'trans_seo_title_' + locale" v-model="form.translations[locale].seo_title" maxlength="60" class="mt-1 p-2 border-2 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" :placeholder="'SEO title in ' + locale" />
                      <p v-if="form.errors[`translations.${locale}.seo_title`]" class="mt-1 text-xs text-red-500">{{ form.errors[`translations.${locale}.seo_title`] }}</p>
                    </div>

                    <!-- Translated Meta Description -->
                    <div class="mb-4">
                      <label :for="'trans_meta_description_' + locale" class="block text-sm font-medium text-gray-700">Meta Description [{{ locale.toUpperCase() }}]</label>
                      <textarea :id="'trans_meta_description_' + locale" v-model="form.translations[locale].meta_description" maxlength="160" rows="3" class="mt-1 p-2 border-2 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" :placeholder="'Meta description in ' + locale"></textarea>
                      <p v-if="form.errors[`translations.${locale}.meta_description`]" class="mt-1 text-xs text-red-500">{{ form.errors[`translations.${locale}.meta_description`] }}</p>
                    </div>
