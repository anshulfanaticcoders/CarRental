...
const slugAuto = ref({})
props.available_locales.forEach((locale) => {
    const existingSlug = `${form.translations?.[locale]?.slug || ''}`.trim()
    // In edit mode, default to manual so existing URLs don't change unexpectedly.
    slugAuto.value[locale] = existingSlug === ''
})
...
watch(() => form.errors, (newErrors) => {
    for (const locale of props.available_locales) {
        if (
            newErrors[`translations.${locale}.title`] ||
            newErrors[`translations.${locale}.slug`] ||
            newErrors[`translations.${locale}.content`] ||
            newErrors[`seo_translations.${locale}.seo_title`] ||
            newErrors[`seo_translations.${locale}.meta_description`] ||
            newErrors[`seo_translations.${locale}.keywords`]
        ) {
            activeLocale.value = locale;
            break;
        }
    ...
    form.post(route('admin.blogs.update', props.blog.id), {
        ...
        onError: (formErrors) => {
            for (const key in formErrors) {
                if (key.startsWith('translations.') || key === 'image' || key === 'slug') {
                    toast.error(formErrors[key], { position: 'top-right', timeout: 7000 });
                }
            }
        },
    });
          ...
          <div class="p-8">
            <div class="flex items-center space-x-4 mb-8">
              ...
              <div>
                ...
                <p class="text-gray-600">Update blog post content and SEO settings</p>
              </div>
            </div>
              ...
              <div class="mb-8">
                <div class="flex justify-between items-center border-b border-gray-200 mb-6 pb-2">
                  ...
                  <div class="flex items-center space-x-2">
                    <label for="is_published_top" class="text-sm font-medium text-gray-700">Publish Post</label>
                    <Switch
                      id="is_published_top"
                      v-model:checked="form.is_published"
                      class="focus:ring-2 focus:ring-customPrimaryColor"
                    />
                    <p v-if="form.errors.is_published" class="text-sm text-red-600">{{ form.errors.is_published }}</p>
                  </div>
                </div>
                ...
                <div class="space-y-6">
                  ...
                  <div>
                    ...
                    />
                    <p v-if="form.errors[`translations.${activeLocale}.title`]" class="mt-2 text-sm text-red-600">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-customPrimaryColor focus:border-customPrimaryColor transition-colors duration-200 bg-gray-50 focus:bg-white"
                    />
                    <p v-if="form.errors[`translations.${activeLocale}.slug`]" class="mt-2 text-sm text-red-600">
                      {{ form.errors[`translations.${activeLocale}.slug`] }}
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    />
                    <p v-if="form.errors[`translations.${activeLocale}.content`]" class="mt-2 text-sm text-red-600">
                  ...
                  <div>
                    ...
                    >
                        ...
                        <div v-else-if="blog.image && !form.image" class="space-y-4">
                            <div class="relative">
                                <img :src="blog.image" alt="Current blog image" class="mx-auto h-48 w-full object-cover rounded-lg border-2 border-gray-200">
              ...
              <div class="flex items-center justify-between pt-6 border-t border-gray-200">
                <div class="flex items-center gap-4">
                  ...
                  <Link :href="route('admin.blogs.index')">
                    <Button type="button" variant="outline">Back to Blogs</Button>
                  </Link>
                </div>
                ...
                <div v-if="form.processing" class="flex items-center text-blue-600">
                  ...
                  <span class="text-sm font-medium">Updating blog post...</span>
                </div>
              </div>
            </form>
          </div>
