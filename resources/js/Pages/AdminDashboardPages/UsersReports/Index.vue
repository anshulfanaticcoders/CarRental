        ...
        <div class="flex-col md:flex">
            <div class="flex-1 space-y-4 p-4 sm:p-6 lg:p-8 pt-6">
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                    <div>
                        <h2 class="text-2xl font-bold tracking-tight text-gray-900">Users Report</h2>
                        <p class="text-sm text-muted-foreground mt-1">Customer registration, activity, and growth analytics</p>
                    </div>
                ...
                <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    <Card class="border-l-4 border-l-blue-500">
                        ...
                        <CardContent>
                            ...
                            <p class="text-xs mt-1" :class="totalCustomersGrowth >= 0 ? 'text-emerald-600' : 'text-red-500'">
                                {{ totalCustomersGrowth >= 0 ? '+' : '' }}{{ totalCustomersGrowth }}% from last period
                            </p>
                        </CardContent>
                    </Card>
                    <Card class="border-l-4 border-l-emerald-500">
                        ...
                        <CardContent>
                            ...
                            <p class="text-xs mt-1" :class="activeCustomersGrowth >= 0 ? 'text-emerald-600' : 'text-red-500'">
                                {{ activeCustomersGrowth >= 0 ? '+' : '' }}{{ activeCustomersGrowth }}% from last period
                            </p>
                        </CardContent>
                    </Card>
                    <Card class="border-l-4 border-l-amber-500">
                        ...
                        <CardContent>
                            ...
                            <p class="text-xs mt-1" :class="newCustomersGrowth >= 0 ? 'text-emerald-600' : 'text-red-500'">
                                {{ newCustomersGrowth >= 0 ? '+' : '' }}{{ newCustomersGrowth }}% vs last period
                            </p>
                        </CardContent>
                    </Card>
                </div>
                ...
                <div class="grid gap-4 xl:grid-cols-2">
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Active vs new users per {{ selectedReport === 'daily' ? 'day' : selectedReport === 'weekly' ? 'week' : 'month' }}</CardDescription>
                        </CardHeader>
                ...
                <Card v-if="recentActivities && recentActivities.length > 0">
                    <CardHeader>
                        ...
                        <CardDescription>Latest customer activity in the selected period</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <div class="space-y-4">
                            <div
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>All customers registered in the selected period</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <Table>
                            <TableHeader>
                            ...
                            <TableBody>
                                <TableRow
                                    v-for="row in customerReportTableData.data"
                                    :key="row.id"
                                ...
                                >
                                    ...
                                    <TableCell>
                                        <span
                                            ...
                                            :class="userStatusClass(row.status)"
                                        >
                                            {{ row.status }}
                                        </span>
                                    </TableCell>
                                ...
                                <TableRow v-if="!customerReportTableData.data?.length">
                        ...
                        <!-- Pagination -->
                        <div v-if="customerReportTableData.last_page > 1" class="flex items-center justify-between pt-4">
                            <p class="text-sm text-muted-foreground">
                                Page {{ customerReportTableData.current_page }} of {{ customerReportTableData.last_page }}
                                ({{ customerReportTableData.total }} total)
                            </p>
                            <div class="flex items-center gap-1">
                                <Button
                                    variant="outline"
                                    size="sm"
                                    :disabled="customerReportTableData.current_page <= 1"
                                    @click="onPageChange(customerReportTableData.current_page - 1)"
                                >
                                ...
                                <template v-for="page in pageNumbers" :key="page">
                                    <Button
                                        v-if="page !== '...'"
                                        :variant="page === customerReportTableData.current_page ? 'default' : 'outline'"
                                        size="sm"
                                        class="w-8"
                                ...
                                <Button
                                    variant="outline"
                                    size="sm"
                                    :disabled="customerReportTableData.current_page >= customerReportTableData.last_page"
                                    @click="onPageChange(customerReportTableData.current_page + 1)"
                                >
...
const props = defineProps({
    ...
    totalCustomersGrowth: { type: Number, default: 0 },
    activeCustomers: { type: Number, default: 0 },
    activeCustomersGrowth: { type: Number, default: 0 },
    newCustomers: { type: Number, default: 0 },
    newCustomersGrowth: { type: Number, default: 0 },
    monthlyData: { type: Array, default: () => [] },
    weeklyData: { type: Array, default: () => [] },
    dailyData: { type: Array, default: () => [] },
    recentActivities: { type: Array, default: () => [] },
    customerReportTableData: { type: Object, default: () => ({ data: [], current_page: 1, last_page: 1, total: 0 }) },
...
const pageNumbers = computed(() => {
    const current = props.customerReportTableData.current_page;
    const last = props.customerReportTableData.last_page;
