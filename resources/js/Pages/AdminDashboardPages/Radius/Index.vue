        ...
        <div class="flex flex-col gap-4 w-[95%] ml-[1.5rem]">
            ...
            <div class="mt-2 mb-2">
                <div class="flex gap-2">
                    <div class="flex-1">
                        <Input v-model="search" type="text" placeholder="Search by city, state, or country..."
                            class="w-full" />
                    </div>
            ...
            <div class="rounded-md border p-5 mt-[1rem] bg-[#153B4F0D]">
                <Table>
                    ...
                    <TableBody>
                        <TableRow v-for="(combination, index) in combinations.data" :key="index">
                            <TableCell>{{ (combinations.current_page - 1) * combinations.per_page + index + 1 }}
                            </TableCell>
                            ...
                            <TableCell>
                                {{ getRadiusForCombination(combination.city, combination.state, combination.country) ||
                                'N/A' }}
                            </TableCell>
                            <TableCell class="text-right">
                                <div class="flex justify-end gap-2">
                                    <Button size="sm" variant="outline"
                                        @click="openEditDialog(combination)">Edit</Button>
                ...
                <div class="flex justify-between items-center mt-4">
                    <div>
                        Showing {{ combinations.from || 0 }} to {{ combinations.to || 0 }} of {{ combinations.total }}
                        entries
                    </div>
                    <div class="flex gap-2">
                        <Button :disabled="!combinations.prev_page_url"
                            @click="navigateToPage(combinations.current_page - 1)">
                            Previous
                        </Button>
                        <Button :disabled="!combinations.next_page_url"
                            @click="navigateToPage(combinations.current_page + 1)">
                            Next
                        </Button>
                    </div>
            ...
            <DialogContent class="max-w-xl">
                ...
                <form @submit.prevent="update" class="space-y-4">
                    <div>
                        ...
                        <Input v-model="editForm.city" type="text" placeholder="Enter city" readonly class="bg-gray-100"/>
                    </div>

                    <div>
                        <InputLabel for="state" value="State" />
                        <Input v-model="editForm.state" type="text" placeholder="Enter state" readonly class="bg-gray-100"/>
                    </div>

                    <div>
                        <InputLabel for="country" value="Country" />
                        <Input v-model="editForm.country" type="text" placeholder="Enter country" readonly class="bg-gray-100"/>
                    </div>
