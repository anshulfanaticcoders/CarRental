                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200">
                    <div class="p-8">
                        <div class="flex items-center space-x-4 mb-8">
                            ...
                            <div>
                                <h1 class="text-2xl font-bold text-gray-900">Add API Consumer</h1>
                                <p class="text-gray-600">Register a new company for Vrooem Provider API access</p>
                            </div>
                        </div>
                        ...
                        <form @submit.prevent="submitForm" class="space-y-6">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="md:col-span-2">
                                    ...
                                    <Input id="name" v-model="form.name" type="text" required class="w-full" placeholder="e.g. TravelCo" />
                                ...
                                <div>
                                    ...
                                    <Input id="contact_name" v-model="form.contact_name" type="text" required class="w-full" placeholder="e.g. Mario Rossi" />
                                ...
                                <div>
                                    ...
                                    <Input id="contact_email" v-model="form.contact_email" type="email" required class="w-full" placeholder="e.g. mario@travelco.it" />
                                    <p v-if="form.errors.contact_email" class="mt-1 text-sm text-red-600">{{ form.errors.contact_email }}</p>
                                </div>

                                <div>
                                    <label for="contact_phone" class="block text-sm font-medium text-gray-700 mb-2">Contact Phone</label>
                                    <Input id="contact_phone" v-model="form.contact_phone" type="tel" class="w-full" placeholder="+39 02 1234567" />
                                ...
                                <div>
                                    ...
                                    <Input id="company_url" v-model="form.company_url" type="url" class="w-full" placeholder="https://travelco.it" />
                            ...
                            <div class="flex items-center justify-between pt-6 border-t border-gray-200">
                                <div class="flex items-center gap-4">
                                    <Button type="submit" :disabled="form.processing">
                                        {{ form.processing ? 'Creating...' : 'Create Consumer' }}
                                    </Button>
...
watch(() => form.plan, (newPlan) => {
    form.rate_limit = props.defaultRateLimit?.[newPlan] ?? 60;
});

const submitForm = () => {
    form.post(route('admin.api-consumers.store'), {
        preserveScroll: true,
        onSuccess: () => {
            toast.success('API consumer created successfully!', { position: 'top-right', timeout: 3000 });
        },
        onError: (errors) => {
