                            ...
                            <TableRow v-for="(user,index) in users.data" :key="user.id" class="hover:bg-muted/25 transition-colors">
                                <TableCell class="px-4 py-3">
                                    <Checkbox
                                        ...
                                        @update:checked="(checked) => toggleRow(user.id, checked)"
...
onMounted(async () => {
    ...
    } catch (e) { /* fallback handled by userCountry */ }
});

const userCountry = (user) => {
    const raw = (user?.profile?.country || '').toString().trim();
...
const visibleIds = computed(() => (props.users?.data || []).map((u) => u.id));
