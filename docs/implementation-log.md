...
### 2026-06-20 - Affiliate soft-deleted contact reuse
- Scope: `CarRental` affiliate registration email/phone validation and `affiliate_businesses.contact_email` indexing.
- Decision: active affiliate businesses still block duplicate email/phone, but soft-deleted affiliate businesses no longer make the public register form say an email is already taken. A migration drops the old unique `contact_email` index so a deleted partner row does not block a new active registration at insert time.
- Verification: `php artisan migrate --pretend` showed only `alter table affiliate_businesses drop index affiliate_businesses_contact_email_unique`; `AffiliateRegistrationTest` passed with 15 tests/71 assertions; `PasswordResetTest` passed with 5 tests/13 assertions; Pint passed for touched PHP/test files.

### 2026-06-20 - Affiliate partner/user delete cleanup
...
### 2026-06-25 - Provider checkout stale quote guard
- Scope: `CarRental` Stripe checkout verification, Skyscanner/Trabber offer checkout context, and `vrooem-gateway` vehicle/search cache behavior.
- Decision: external-provider checkout now treats supplier vehicle context as a 15-minute quote, sends pickup/unified-location/driver context from the frontend, and performs a fresh gateway search with `bypass_cache=true` before Stripe Checkout. The fresh quote must match provider/location/vehicle/package/price/currency/extras before Stripe can start, and checkout swaps to the fresh gateway vehicle/search IDs so supplier reservations use current provider data. Gateway vehicle cache TTL is now configurable and defaults to 15 minutes.
- Verification: `php -l` passed for touched Laravel PHP files; `php artisan test tests\Unit\PriceVerificationServiceTest.php` passed with 10 tests/40 assertions; Skyscanner/Trabber focused tests passed with 25 tests/440 assertions; gateway targeted Ruff passed for touched files; gateway `tests/test_cache_service.py`, `tests/test_search_api.py`, and `tests/test_search_service.py` passed with 13 tests total; `npm run build` passed with existing Vite/Browserslist/lottie/chunk warnings. Full gateway Ruff still reports pre-existing repo-wide lint debt outside this change.
- Follow-ups: redeploy both Laravel and gateway for the guard to apply in production; leave `vrooem-gateway/data/unified_locations.json` out of this change unless a separate location refresh is intended.

### 2026-06-25 - Payment status return-to-search link
- Scope: `CarRental` Stripe checkout payload and booking status page.
- Decision: checkout now stores the local search URL and location IDs in full metadata/provider metadata, and the failed/cancelled status page receives a server-built `search_url` instead of relying only on `sessionStorage`. Existing cancelled bookings can rebuild a localized `/en/s?...` link from booking dates, times, provider, driver age, and location metadata.
- Verification: `php -l` passed for touched checkout/status PHP files; `php artisan test tests\Unit\StripeCheckoutStatusReturnSearchTest.php tests\Feature\StripeCheckoutStatusReturnSearchTest.php` passed with 3 tests/23 assertions; `npm run build` passed with existing Vite/Browserslist/lottie/chunk warnings.

### 2026-06-25 - Surprice live booking detail pricing parity
- Scope: `CarRental` booking data normalizer and customer booking details page.
- Decision: booking details now prefer confirmed `provider_pricing` deposit/excess values over stale raw benefit snapshots, while preserving real zero values for FDW excess/theft excess. External provider deposit display now wins over internal security-deposit fallback.
- Verification: Surprice live supplier booking `BK2026060626` / supplier ref `SCKH6IYOSPZW` completed through Stripe sandbox with FDW, navigation, and additional driver. Browser smoke confirmed booking details show total `EUR 294.32`, paid `EUR 38.39`, due `EUR 255.93`, extras, customer details, support references, and deposit/excess `EUR 250.00` / `EUR 0.00` / `EUR 0.00`. `npm run build` passed with existing Vite/Browserslist/lottie/chunk warnings. Direct `node --test tests\js\bookingDataPricingBreakdown.test.js` remains blocked by the repo's existing Vite alias setup for `@/utils`.

### 2026-06-29 - Vendor vehicle listing validation feedback
- Scope: `CarRental` vendor vehicle create/edit forms and vehicle store/update validation.
- Decision: vehicle contact phone validation now accepts formatted international numbers up to 30 characters. Create-listing server validation errors now show the exact Laravel message, jump back to the affected step, and highlight the field instead of only showing the generic final-submit toast.
- Verification: `php -l` passed for touched vehicle controllers; `php artisan test --filter=VendorVehicleLocationSyncTest` passed with 3 tests/34 assertions; `npm run build` passed with existing Browserslist/Vite/lottie/chunk warnings.

### 2026-06-29 - Vendor vehicle create numeric guardrails
...
### 2026-07-02 - Yesaway provider integration bootstrap
- Scope: `vrooem-gateway` Yesaway OTA XML adapter/config/tests and `CarRental` checkout/provider contract support.
- Decision: added Yesaway as an isolated gateway supplier using OTA XML Basic Auth, dynamic branch refresh, rate plans from the Yesaway POA sheet, normalized availability products/extras/coverage/deposit data, booking XML generation, cancellation XML generation, and location-sync registration. Laravel now blocks Yesaway checkout unless selected supplier rate context and required driver contact/license/address fields are present. Generic booking extras now preserve all provider-supplied package types such as `FULL`.
- Verification: raw production endpoint checks confirmed the correct Yesaway endpoint is `/services` and returns branch/availability XML for `BKK01` with products, coverage, deposit, and customer-selectable extras. Gateway `tests\test_yesaway_adapter.py` passed with 3 tests and file-scoped Ruff passed. Laravel `php -l` passed for touched PHP files; `php artisan test --filter=ProviderBookingContractTest` passed with 6 tests/15 assertions; `npm run build` passed with existing Browserslist/lottie/chunk warnings.
- Follow-ups: set Yesaway env vars in Coolify without exposing secrets, redeploy gateway/Laravel, run location sync excluding unrelated `data/unified_locations.json` changes from source control, then smoke a future-date search and explicit test booking/cancel only after approval.

### 2026-07-06 - Surprice broker order reference
...
### 2026-08-17 - Booking/payment/supplier integrity hardening
- Scope: `CarRental` checkout pricing, Stripe paid-session fulfilment, supplier reservation/cancellation, partner API, customer/admin booking presentation, and `vrooem-gateway` reservation replay protection.
- Decision: supplier operations now share a per-booking lock and require paid eligibility; gateway cancellation must explicitly confirm success before local cancellation. Added a durable paid-session reconciliation ledger and scheduler. Refunds remain strictly manual: no Stripe refund execution was added, and refunded/suspect payments route to manual review. Partner pricing/eligibility/add-ons/time handling now share server-trusted rules; customer/admin status and receipt projections are exhaustive and safer.
- Verification: targeted Laravel suites passed with 91 tests/1,891 assertions after rerunning the corrected admin trace-note test; Node platform-pricing tests passed with 5 tests; production Vite/SSR build passed; migration pretend passed; Pint passed. Gateway booking-service tests passed with 12 tests and file-scoped Ruff passed.
- Follow-ups: deploy both repos, run `php artisan migrate`, restart queue workers, and confirm the scheduler is active. Gateway replay storage uses Redis, so production Redis persistence/retention must remain enabled.

### 2026-08-17 - Release closure and non-destructive migration gate
- Scope: amount-mismatch customer presentation, admin booking actions, customer-safe locations, internal-location merge, Trabber click idempotency, deployment migrations, notification tests, and vehicle-deletion evidence retention.
- Decision: customer pages now show Stripe's actual captured amount under review and suppress conversion tracking on mismatches. Invalid admin retry/cancel actions are hidden. Release migrations never delete/drop data in `up()`; legacy provider tables stay untouched, affiliate duplicates fail preflight, and historical Trabber duplicates are preserved behind one canonical atomic-upsert row. Mixed searches require an exact internal location hash. Vehicle deletion preserves booking damage records and images.
- Verification: production Vite/SSR build passed; migration dry-run passed; 8 Node pricing/presentation tests passed; 15 Trabber integration tests passed; the broad Laravel run passed 543 tests and exposed 9 unrelated/stale failures, then all 9 and their related coverage passed in a 33-test/171-assertion rerun. Manual-refund and non-destructive-migration guards passed with 1,393 assertions. Pint passed for touched PHP files.
- Follow-ups: production deployment still requires backup, migrations, queue restart, scheduler/Redis checks, and a Stripe-test-mode staging booking/cancellation smoke. No automatic refund execution exists.

### 2026-08-17 - Admin email flood guard
- Scope: Laravel test mail transport and booking/payment operational admin notifications.
- Decision: PHPUnit now force-overrides inherited mail settings, and the application/test base class fail closed to Laravel's in-memory array transport under `APP_ENV=testing` or whenever the selected database name contains `test`. Automatic paid-session reconciliation is production-only. Booking-created, correction, supplier-failed, supplier-unknown, and partner-reversal admin alerts now persist a per-incident key and send at most once per rolling 24 hours.
- Verification: mail-isolation and dedupe regressions passed, including a deliberately mislabelled environment connected to `carrental_testing`; the affected booking/reservation/reversal/accounting suites passed with 45 tests/181 assertions. Pint and `git diff --check` passed for the incident changes.
- Follow-ups: deploy the Laravel app and restart queue workers for production dedupe to take effect. Refund handling remains manual-only.

### 2026-08-18 - Internal-fleet booking pipeline audit
- Scope: `CarRental` local Stripe-free internal booking fulfilment, accounting, availability, price-integrity, terminal-state, and cancellation coverage.
- Decision: added `InternalFleetBookingPipelineTest` with constructed Stripe sessions only; no live Stripe call or charge is possible in the suite. Paid guest/registered/deposit/extras, webhook/success idempotency, overlap rejection, tampered-price rejection, expired/forced-failure payload states, and cancellation were exercised.
- Findings: paid internal cancellation stores `cancelled` and reason but does not mark `manual_refund_required`; `booking_extras` has no currency column. Internal prices also bypass `PROVIDER_MARKUP_PERCENT`, so the 15% provider-markup rule is external-provider only.
- Verification: `php artisan test --filter=InternalFleetBookingPipelineTest --testdox` passed with 8 tests/81 assertions; companion Stripe/price/recovery suites passed with 37 tests/107 assertions; file-scoped Pint passed. Broad Pint still reports existing repository-wide formatting debt outside this audit.
